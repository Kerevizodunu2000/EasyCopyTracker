...
BACKUP_FILE = os.path.join(BASE_DIR, "session_backup.json")
RECOVERY_FILE = os.path.join(BASE_DIR, "recovery_pending.json")
LEGACY_FILE = os.path.join(BASE_DIR, "data.json")
LOG_FILE = os.path.join(BASE_DIR, "copytracker.log")
PID_FILE = os.path.join(BASE_DIR, "copytracker.pid")
HOST = "127.0.0.1"
PORT = 8765
...
def save_backup():
    """RAM listesinin gölge kopyası — çökme sonrası kurtarma için."""
    with _lock:
        try:
            _write_json(BACKUP_FILE, {"items": _items, "next_id": _next_id, "saved_at": now_iso()})
        except OSError as e:
...
def load_archive():
    ...
    if not isinstance(d, dict) or not isinstance(d.get("items"), list):
        return
    items = [e for e in d["items"] if isinstance(e, dict) and "text" in e]
    for e in items:
        e.setdefault("aid", 0)
        e.setdefault("archived_at", now_iso())
        e.setdefault("collection_name", "Genel")
        e.setdefault("url", None)
        e.setdefault("is_link", e.get("url") is not None)
...
def check_recovery():
    ...
    if os.path.exists(BACKUP_FILE):
        try:
            os.replace(BACKUP_FILE, RECOVERY_FILE)
        except OSError:
            pass
    d = _read_json(RECOVERY_FILE)
    if isinstance(d, dict) and isinstance(d.get("items"), list) and d["items"]:
        _recovery = [it for it in d["items"] if isinstance(it, dict) and "text" in it]
        if _recovery:
            log(f"Önceki oturumdan {len(_recovery)} kurtarılabilir öğe bulundu.")
        else:
            _recovery = None
...
def _host_of(url):
    try:
        return urlparse(url).netloc.rsplit("@", 1)[-1].split(":")[0].lower()
    except ValueError:
        return ""
...
def as_web_link(text):
    ...
    if p.scheme not in ("http", "https") or not p.netloc:
        return None
    host = p.netloc.rsplit("@", 1)[-1].split(":")[0]
...
_title_sem = threading.Semaphore(3)
...
def add_item(text):
    ...
    save_backup()
    if url:
        threading.Thread(target=_fetch_title, args=(item["id"], url), daemon=True).start()
        ...
        try:
            ...
            if status == "ok":
                ...
                if kind in ("skip", "filtered"):
                    if kind == "filtered":
                        log(f"Filtre nedeniyle kaydedilmedi: {short(text, 60)}")
                    continue
                ...
                if kind == "dup":
                    ...
                    log(f"#{item['id']} LINK kaydedildi ({col}): {item['url']}")
...
@app.before_request
def csrf_guard():
    """CSRF koruması: başka bir sitedeki sayfa localhost'a POST atamasın."""
...
def api_settings():
    ...
    with _lock:
        if "filter_mode" in body:
            ...
            _settings["filter_mode"] = body["filter_mode"]
        ...
        if "custom_domains" in body:
            ...
            _settings["custom_domains"] = [_norm_domain(str(s)) for s in doms if str(s).strip()][:50]
    ...
    if "capture_enabled" in body:
        set_capture(bool(body["capture_enabled"]))
    ...
    if action == "restore":
        with _lock:
            for it in _recovery:
                ...
                it.setdefault("title", None)
                if it.get("collection") not in {c["id"] for c in _settings["collections"]}:
                    it["collection"] = 1
                _next_id += 1
                _items.append(it)
            count = len(_recovery)
